...
| **Change Queue** | The Project's append-only triage list of sponsor observations (defects, amendment wishes, ideas) captured during or between sessions. Status flow QUEUED → CLASSIFIED → CONFIRMED → IN_PROGRESS → DELIVERED (PARKED = the sponsor's stash, reachable pre-run and reopenable with classification kept). Entries carry a sponsor-set priority (HIGH/NORMAL/LOW) — an ordering signal only, never a routing input. Entries are never removed. | `state.changeQueue[]` | Live triage record (PROP-054 B.2) |
