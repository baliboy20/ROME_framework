...
export class ScrapingService {
  ...
  async scrapeUrl(url: string, options?: ScrapingOptions): Promise<ScrapedContent> {
  ...
  private async extractArticle(url: string, requestId: string): Promise<any> {
    try {
  ...
  private processArticleData(article: any, url: string, requestId: string): ScrapedContent {
  ...
  private async puppeteerScrape(url: string, options: ScrapingOptions, requestId: string): Promise<ScrapedContent> {
    if (!this.browser) {
  ...
  private cleanupHtml(html: string, selectors: string[]): string {
  ...
  private convertToMarkdown(html: string, title?: string, author?: string, date?: string, url?: string): string {
    let markdown = '';
