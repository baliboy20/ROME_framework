...
import type { EmailFilterOptions } from '@/types/gmail.types.js';
...
export class EmailController {
   ...
   */
  fetchEmails = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      for (let i = 0; i < processedEmails.length; i++) {
        ...
        try {
          ...
          const digestId = await this.storageService.saveEmailDigest(digestData as any);
  ...
  listEmails = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      query.limit = req.query.limit ? parseInt(req.query.limit as string) : 50;
      query.skip = req.query.skip ? parseInt(req.query.skip as string) : 0;
   ...
   */
  getEmailLinks = async (req: Request, res: Response): Promise<void> => {
      ...
      const response = {
        ...
        links: {
          ...
          summary: {
            ...
            flutterDomains: Object.keys(linksByDomain).filter(domain => 
              linksByDomain[domain]?.some((link: any) => link.isFlutterRelated)
            ).length
          }
        }
      };
   ...
   */
  processSelectedLinks = async (req: Request, res: Response): Promise<void> => {
      ...
      for (let i = 0; i < selectedLinks.length; i++) {
        ...
        try {
          ...
          if (scrapedContent.success) {
            ...
            // Save to file system
            const savedFile = await fileSystemService.saveMarkdownFile(
  ...
  private enrichLinksWithMetadata(allLinks: string[], flutterLinks: string[], requestId?: string) {
  ...
  private groupLinksByDomain(enrichedLinks: any[], requestId?: string) {
    ...
    const sortedDomains = Object.keys(grouped).sort((a, b) => {
      const aFlutterCount = grouped[a]?.filter((link: any) => link.isFlutterRelated).length || 0;
      const bFlutterCount = grouped[b]?.filter((link: any) => link.isFlutterRelated).length || 0;
    ...
    const result: { [domain: string]: any[] } = {};
  ...
  private categorizeLink(url: URL, isFlutterRelated: boolean): string {
    const domain = url.hostname.toLowerCase();
    const path = url.pathname.toLowerCase();

    if (isFlutterRelated) {
      if (domain.includes('medium.com')) return 'flutter-article';
      if (domain.includes('flutter.dev')) return 'flutter-docs';
      if (domain.includes('dart.dev')) return 'dart-docs';
      if (domain.includes('pub.dev')) return 'flutter-package';
  ...
  private generateDisplayTitle(url: URL): string {
    try {
      const domain = url.hostname.toLowerCase();
  ...
  private estimateReadTime(url: string): string {
    try {
      const domain = new URL(url).hostname.toLowerCase();
      
      if (domain.includes('medium.com')) return '5-8 min';
      if (domain.includes('dev.to')) return '3-6 min';
      if (domain.includes('flutter.dev')) return '10-15 min';
      if (domain.includes('dart.dev')) return '8-12 min';
      if (domain.includes('github.com')) return '2-5 min';
   ...
   */
  listSavedArticles = async (req: Request, res: Response): Promise<void> => {
   ...
   */
  getSavedArticle = async (req: Request, res: Response): Promise<void> => {
   ...
   */
  deleteSavedArticle = async (req: Request, res: Response): Promise<void> => {
