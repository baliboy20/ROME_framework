...
import type { ScrapingOptions } from '@/types/scraper.types.js';
...
export class ScrapingController {
   ...
   */
  startBatchScraping = async (req: Request, res: Response): Promise<void> => {
    try {
   ...
   * Scrape a single URL
   */
  scrapeSingle = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (saveToDatabase && result.content) {
        try {
          ...
          const { articleId: id } = await this.storageService.saveArticle(
            result.content,
            {
   ...
   */
  saveBatchResults = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      for (const result of successfulResults) {
        try {
          const { articleId } = await this.storageService.saveArticle(
            result.content!,
            emailInfo || {
              messageId: `batch-${batchId}`,
   ...
   */
  cleanupBatches = async (req: Request, res: Response): Promise<void> => {
    try {
   ...
   */
  private setupAutoSave(batchId: string, emailInfo: { messageId: string; subject: string; date: Date }): void {
