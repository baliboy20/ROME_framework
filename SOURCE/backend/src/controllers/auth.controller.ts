...
export class AuthController {
   ...
   */
  initializeAuth = async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  handleCallback = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (req.session && (req.session as any).authState) {
        if ((req.session as any).authState !== state) {
          res.status(400).json({
            error: 'Invalid state',
            code: 'STATE_MISMATCH',
      ...
      if (req.session) {
        (req.session as any).userId = authorizedUser.userId;
        (req.session as any).userEmail = authorizedUser.email;
      }
