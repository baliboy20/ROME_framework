...
import type { EmailFilterOptions, GmailMessage } from '../../types/gmail.types';
...
describe('GmailService', () => {
  ...
  describe('buildGmailQuery', () => {
    it('should build basic query with subject filter', () => {
      ...
      const query = (gmailService as any).buildGmailQuery(options);
  ...
  describe('fetchMessageList', () => {
    it('should fetch message list with pagination', async () => {
      ...
      const result = await (gmailService as any).fetchMessageList(
        mockGmail, 
        'test-query', 
        10
      );
  ...
  describe('fetchFullMessages', () => {
    it('should fetch full messages in batches', async () => {
      ...
      const result = await (gmailService as any).fetchFullMessages(mockGmail, messageIds);
  ...
  describe('extractEmailContent', () => {
    it('should extract email headers correctly', () => {
      ...
      const result = (gmailService as any).extractEmailContent(mockMessage);
  ...
  describe('extractLinks', () => {
    it('should extract links from HTML content', () => {
      ...
      const result = (gmailService as any).extractLinks(htmlContent);
    ...
    it('should extract links from plain text', () => {
      ...
      const result = (gmailService as any).extractLinks(plainText);
    ...
    it('should handle empty content', () => {
      const result = (gmailService as any).extractLinks('');
  ...
  describe('fetchDigests', () => {
    it('should fetch and process email digests', async () => {
      ...
      mockAuthService.getAuthorizedClient.mockResolvedValue(mockAuthClient as any);
    ...
    it('should handle authentication errors', async () => {
      mockAuthService.getAuthorizedClient.mockRejectedValue(
      ...
      await expect(
        gmailService.fetchDigests(mockUserId, filterOptions)
      ).rejects.toThrow('Failed to fetch email digests: Authentication failed');
    });
    ...
    it('should handle Gmail API errors', async () => {
      ...
      await expect(
        gmailService.fetchDigests(mockUserId, filterOptions)
      ).rejects.toThrow('Failed to fetch email digests: Gmail API error');
    });
  });
  ...
  describe('Cache Management', () => {
    it('should clear cache when requested', () => {
      ...
      (gmailService as any).cache.set('test-key', 'test-value');

      expect((gmailService as any).cache.size).toBe(1);

      gmailService.clearCache();

      expect((gmailService as any).cache.size).toBe(0);
    });
    ...
    it('should generate consistent cache keys', () => {
      ...
      const key1 = (gmailService as any).getCacheKey(userId, query);
      const key2 = (gmailService as any).getCacheKey(userId, query);
