...
`);
  ...
  private async ingestDirectory(directoryPath: string, pattern?: string): Promise<void> {
    this.logger.info(`Ingesting directory: ${directoryPath}`);
    ...
    try {
      ...
      console.error(`❌ Failed to ingest directory ${directoryPath}:`, error instanceof Error ? error.message : 'Unknown error');
      process.exit(1);
    }
  }
