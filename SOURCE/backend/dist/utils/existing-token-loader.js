...
export class ExistingTokenLoader {
     ...
     */
    static async loadFromTokenFile(tokenFilePath) {
        try {
     ...
     */
    static async migrateExistingToken(authService, userId, tokenFilePath) {
        try {
 ...
 */
export async function migrateExistingGmailToken(authService) {
    ...
    try {
        const migrated = await ExistingTokenLoader.migrateExistingToken(authService, defaultUserId, tokenPath);
        if (migrated) {
...
//# sourceMappingURL=existing-token-loader.js.map