...
export class StorageService {
    ...
    async initialize() {
        ...
        this.emailDigestsCollection = this.db.collection('email_digests');
    ...
    async saveArticle(scrapedContent, emailInfo) {
    ...
    async saveEmailDigest(emailDigest) {
        await this.initialize();
    ...
    async updateEmailDigestWithArticles(messageId, articleIds) {
        await this.initialize();
        await this.emailDigestsCollection.updateOne({ messageId }, {
            $set: {
                articles: articleIds,
                status: 'processed',
    ...
    async updateArticle(id, updates) {
        await this.initialize();
        const objectId = typeof id === 'string' ? new ObjectId(id) : id;
        const result = await this.articlesCollection.updateOne({ _id: objectId }, {
            $set: {
                ...updates,
                lastUpdated: new Date()
            }
    ...
    async saveToFile(scrapedContent, emailDate) {
    ...
    createMarkdownWithMetadata(content) {
        const metadata = [
            '---',
            `title: "${content.title}"`,
            `url: "${content.url}"`,
