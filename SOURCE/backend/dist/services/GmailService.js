...
export class GmailService {
    ...
    async fetchDigests(userId, options = {}) {
    ...
    async fetchSingleEmail(userId, messageId) {
        try {
            ...
            const [processedEmail] = await this.processMessages([message.data]);
    ...
    buildGmailQuery(options, requestId) {
    ...
    async fetchMessageList(gmail, query, maxResults, requestId) {
    ...
    async fetchFullMessages(gmail, messageIds, requestId) {
        ...
        for (let i = 0; i < messageIds.length; i += batchSize) {
            ...
            try {
                ...
                const validMessages = batchResults.filter(msg => msg);
    ...
    async processMessages(messages, requestId) {
    ...
    extractLinks(content, requestId) {
        ...
        for (let i = 0; i < links.length; i++) {
            const link = links[i];
            if (!link)
                continue; // Skip if link is undefined
    ...
    convertHtmlToMarkdown(htmlContent, requestId) {
    ...
    cleanHtmlForMarkdown(htmlContent, requestId) {
    ...
    cleanMarkdown(markdown, requestId) {
