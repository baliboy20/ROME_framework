...
describe('AuthService', () => {
    ...
    beforeEach(() => {
        ...
        authService.oAuth2Client = mockOAuth2Client;
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('handleCallback', () => {
        ...
        beforeEach(() => {
            ...
            authService.stateStore.set(mockState, {
                state: mockState,
        ...
        it('should reject invalid state', async () => {
            await expect(authService.handleCallback(mockCode, 'invalid-state')).rejects.toThrow('Invalid or expired state parameter');
        });
        it('should reject expired state', async () => {
            const expiredState = 'expired-state';
            authService.stateStore.set(expiredState, {
                state: expiredState,
                codeVerifier: 'test-verifier',
                expiresAt: new Date(Date.now() - 1000) // 1 second ago
            });
            await expect(authService.handleCallback(mockCode, expiredState)).rejects.toThrow('Authentication state has expired');
        });
        it('should clean up state after successful callback', async () => {
            ...
            await expect(authService.handleCallback(mockCode, mockState)).resolves.toEqual({
            ...
            expect(authService.stateStore.has(mockState)).toBe(false);
    ...
    describe('refreshAccessToken', () => {
        ...
        it('should throw error when no refresh token available', async () => {
            ...
            await expect(authService.refreshAccessToken(mockUserId)).rejects.toThrow('No refresh token available');
        });
    ...
    describe('getAuthorizedClient', () => {
        ...
        it('should throw error when user not authenticated', async () => {
            // @ts-ignore
            mockTokenStorage.load.mockResolvedValue(null);
            await expect(authService.getAuthorizedClient(mockUserId)).rejects.toThrow('User not authenticated');
        });
    ...
    describe('State Management', () => {
        it('should clean up expired states automatically', async () => {
            ...
            authService.stateStore.set(validState, {
                state: validState,
            ...
            const stateStore = authService.stateStore;
            const now = new Date();
