...
describe('StorageService', () => {
    ...
    let mockClient;
    let mockDb;
    let mockArticlesCollection;
    let mockEmailDigestsCollection;
    let mockFs;
    ...
    beforeEach(() => {
        ...
        mockDb = {
            ...
            collection: jest.fn().mockImplementation((name) => {
                if (name === 'articles')
                    return mockArticlesCollection;
                if (name === 'email_digests')
                    return mockEmailDigestsCollection;
                return {};
            })
        };
        mockClient = {
    ...
    describe('saveArticle', () => {
        ...
        it('should create directory structure for file storage', async () => {
            ...
            expect(mockFs.mkdir).toHaveBeenCalledWith(expect.stringContaining('data/articles'), { recursive: true });
        });
        it('should generate markdown with metadata', async () => {
            await storageService.saveArticle(mockScrapedContent, mockEmailInfo);
            expect(mockFs.writeFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('---'), 'utf-8');
        });
    });
    describe('saveEmailDigest', () => {
        const mockEmailDigest = {
    ...
    describe('updateEmailDigestWithArticles', () => {
        it('should update email digest with article IDs', async () => {
            ...
            expect(mockEmailDigestsCollection.updateOne).toHaveBeenCalledWith({ messageId }, {
                $set: {
                    articles: articleIds,
                    status: 'processed',
    ...
    describe('updateArticle', () => {
        it('should update article with new data', async () => {
            const updates = { title: 'Updated Title', category: 'flutter' };
            // @ts-ignore
            ...
            expect(result).toBe(true);
            expect(mockArticlesCollection.updateOne).toHaveBeenCalledWith({ _id: mockArticleId }, {
                $set: {
                    ...updates,
    ...
    describe('Private Methods', () => {
        describe('formatBytes', () => {
            it('should format bytes correctly', () => {
                const formatBytes = storageService.formatBytes;
        ...
        describe('createMarkdownWithMetadata', () => {
            it('should create properly formatted markdown with metadata', () => {
                ...
                const result = storageService.createMarkdownWithMetadata(mockContent);
