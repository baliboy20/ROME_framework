...
timeout 60s bash -c 'until curl -f http://localhost:8080/v1/schema > /dev/null 2>&1; do 
...
timeout 60s bash -c 'until curl -f http://localhost:3000/health > /dev/null 2>&1; do 
...
echo "   Weaviate UI:   http://localhost:8080/v1/schema"
echo ""
