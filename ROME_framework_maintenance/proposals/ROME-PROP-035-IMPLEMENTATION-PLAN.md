...
**2.3 Orchestrator (Roma) skeleton** — EDIT `roma/ROBOT.md` + `modes/orchestrator.md` into the active driver: phase state machine, sub-agent dispatch, fan-out/join, gate invocation. Drives only; the guard enforces. **✅ deterministic substrate DONE** — `rome-core/orchestrator/README.md` defines the operating contract (createState → dispatch → invoke → processReturn → recordGateVerdict → guard.advance); dispatch/audit recording in `subagent.js`. *Remaining (Stage 3): the LLM-facing Roma mode rewrite that calls this substrate against live sub-agents.*

**2.4 Sub-agent definition format** — define how a robot folder (`ROBOT.md`+`modes/`+`skills/`) is loaded as a sub-agent with scoped tools and a **structured return** schema (artifacts + summary + traceability deltas). **✅ DONE** — `subagent.js`: `loadRoleSpec()` assembles system prompt (identity + active mode + return contract) and scoped skill list from a real robot-plugin folder; `validateReturn()` enforces the structured-return schema; `processReturn()` records completion into state (completion = return = record, §6b) and merges traceability deltas; `coverage()` reports requirement coverage. **17/17 tests pass** (incl. loading the real `pma` role). Combined orchestrator suite: **37/37** via `tests/run.cjs`.
