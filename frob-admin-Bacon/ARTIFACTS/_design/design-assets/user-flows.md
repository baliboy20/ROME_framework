...
| **Design system** | `design-system.md` (this directory) — **two brand tracks (DEV-1, sponsor-directed AIB-P3 redirect, extended)**: Flows 1, 2, 5 and their customer-webapp/editor surfaces render from **Track A — Forest** tokens (Syne/DM Sans) per TDR-15; **Flow 3 (guide day-of, G1–G13) and Flow 4 (owner admin, A1–A20) both render from Track B — Internal Apps Parchment** tokens (Playfair Display/Plus Jakarta Sans/mono, pink/lime/cyan/orange status accents), grounded in the sponsor-supplied mockups at `_user_input/design-mockups/admin-system/` and `_user_input/design-mockups/guide-system/` (identical `_ds` token bundle in both). Rendering target (static/island/SPA/native) per **TDR-13**. |
