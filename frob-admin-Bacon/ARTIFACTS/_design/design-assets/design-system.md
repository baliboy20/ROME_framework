...
- **A19 Booking browser**: read-only list-detail — search + results DataTable, full record (attendees, one emergency contact, payment as amount+status+provider-ref **never a card number**, waiver/T&C/consent timestamps, status history).
