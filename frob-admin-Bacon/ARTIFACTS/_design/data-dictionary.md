...
- **`message`** *(New)* — `id`, `message_type`(enum), `recipient`, `event`, `idempotency_key`(unique), `provider`(plain string; `resend` per CHG-008, or `cloudflare-email` as the DR-18 rollback, or `debug` locally. **`postmark` is retired — TDR-09 superseded by DEV-5, 2026-07-29.** A row not yet dispatched records `pending`; `send()` writes the real transport on dispatch. NOTE the DDL still declares `DEFAULT 'postmark'` (0001_init.sql) — unreachable from application code since every INSERT site supplies the column explicitly, and left in place because rebuilding the table is disproportionate; see migration 0010), `provider_ref?`, `status`(enum), `template_id?`(FK `email_templates`, set when the send was rendered from a template), `created_at`, `sent_at?`. **The one send-log for every module's outbound email — EML's `sent_emails` retires into this (F4/§4): content-assembly modules render, then hand off to the shared `send()`/NOTIF01 path for dispatch + idempotency + delivery-status. No separate send table.**
